...
- **Owner:** unassigned
- **Depends on:** T6
- **Goal:** `app/analysis/gate.py` decides, per entry, whether the data is complete enough to judge - so that a defective export is not surfaced as suspicion. Three verdicts: `analyze`, `insufficient_data`, `out_of_scope` (balances, carryforwards, period aggregates, `AfA`-style depreciation markers). Deterministic metrics decide the clear cases at zero model cost; only borderline entries get one cheap-model call on a ≤400-token summary (shape, field-completeness counts, party stats) - never the full brief, whose tokens would cost what the analysis costs. Prompt: `agents/PROMPTS.md` §1.

  **The absence taxonomy is this task's core.** A missing goods receipt is simultaneously incomplete data *and* the finding itself. A gate that reads absence as a reason to discard would delete exactly the entries worth reading. Three distinct classes, and the distinction is what makes the gate safe:

  - **Missing identity** - nothing dates the entry, no amount is stated, no counterparty is named, no document reference ties it to a source document. Nobody, human or model, can form a view. → `insufficient_data`.
  - **Missing expected companion** - the entry is fully identified, but a document that bookkeeping grammar or this dossier's own peers say accompanies an entry like it is absent. → **forced `analyze`**, with the absence passed to the analyst as a stated fact.
  - **Missing optional detail** - a field that peer entries commonly lack too. → neutral, no bearing on the verdict.

  So `insufficient_data` requires failing the identity check **and** having no expected-companion absence. A well-identified vendor invoice with no receipt cannot be gated out by construction. The bar for `insufficient_data` is *unjudgeable*, not *imperfect*.

  Completeness is evaluated **per entry, not per record.** A journal row with no counterparty is fine when the invoice it is document-joined to names one; that is the whole reason the entry is assembled before anything looks at it.

  Expected companions come from two sources, in this order. First, **learned** from T5's peer-shape statistics ("412 of the 583 entries with this shape carry a `goods_receipt`; this one does not") - this generalizes to any dossier, including document types nobody anticipated. Second, a small explicit table of **document grammar** for the blind spot the learned version has: when a dossier is systematically missing a companion, or the deficient shape is itself common, or a small dossier has too few peers to compare against. That table may encode only universal bookkeeping structure - a posting expects a document reference (GoBD *Belegprinzip*), a goods movement expects a corresponding posting, a capitalization expects an asset record - stated purely as "record type X is normally accompanied by record type Y or edge type Z".

  **Why this table is not the prefilter returning.** `prefilter.py` *selected* entries and dropped the rest, so its rule list bounded what could ever be found. This table can only *prevent* dropping. Delete every row of it and the system still analyzes everything that passes the identity check - it merely loses a safety net. A rule that can only route work toward the analyst cannot overfit an outcome. Enforce that asymmetry in code: the table's only permitted effect is upgrading a verdict to `analyze`, never downgrading one, and never touching severity.
...
- **Acceptance:** a test proves a well-identified entry missing an expected companion is always routed to `analyze`, whichever source flagged the companion, and that no combination of inputs lets the document-grammar table downgrade a verdict. The gate errs open - any doubt routes to `analyze`, and a test proves an entry the cheap model returns garbage for is analysed rather than dropped. A seeded-random sample of gated-out entries is analysed anyway on every run and reported, so a systematically wrong gate is visible instead of invisible; the seed is fixed so the sample is reproducible. Per-verdict counts are recorded on the analysis run and `insufficient_data` entries are persisted with their reasons - an entry with no document reference is a GoBD compliance observation in its own right, not a silent drop. The gate never reasons about fraud: a test fails if a fraud scenario, domain keyword list or amount threshold appears in the module or its prompt, and the document-grammar table is asserted to contain no amounts, no dates, no keyword strings and no party names - record types and edge types only.
...
- **No fraud scenario may be encoded in code or in a prompt** - no keyword lists, no amount thresholds, no named patterns, no worked examples of fraud. Prompts direct attention; the model supplies the judgement. See `agents/PROMPTS.md`, which holds every prompt in the pipeline and the doctrine behind them; a prompt change is an orchestrator decision. Checks about *data quality* are exempt, because they assert nothing about wrongdoing - but a data-quality rule may only ever route an entry *toward* analysis, never away from it.
